            ...
            modalBody.innerHTML = `
                <div class="full-page-modal-content">
                    ...
                    <div class="modal-text-content">
                        ...
                        ${(data.challengesList && data.solutionsList) ? `
                        <div class="modal-section" style="margin-bottom: 2rem;">
                            <h4 style="margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem;"><i class="fas fa-balance-scale accent"></i> Challenges & Solutions</h4>
                            <div class="cs-grid">
                                <div class="cs-box">
                                    <h5 style="color: #f87171; margin-bottom: 0.8rem; font-size: 1.05rem; display: flex; align-items: center; gap: 0.5rem;"><i class="fas fa-exclamation-triangle"></i> Challenges</h5>
                                    <ul style="list-style-type: none; padding: 0; color: var(--text-muted); font-size: 0.95rem;">
                                        ${data.challengesList.map(c => `<li style="margin-bottom: 0.5rem; position: relative; padding-left: 1.2rem;"><span style="position: absolute; left: 0;">⚠️</span> ${c}</li>`).join('')}
                                    </ul>
                                </div>
                                <div class="cs-box">
                                    <h5 style="color: #4ade80; margin-bottom: 0.8rem; font-size: 1.05rem; display: flex; align-items: center; gap: 0.5rem;"><i class="fas fa-check-circle"></i> Solutions</h5>
                                    <ul style="list-style-type: none; padding: 0; color: var(--text-muted); font-size: 0.95rem;">
                                        ${data.solutionsList.map(s => `<li style="margin-bottom: 0.5rem; position: relative; padding-left: 1.2rem;"><span style="position: absolute; left: 0;">✅</span> ${s}</li>`).join('')}
                                    </ul>
                                </div>
                            </div>
                        </div>` : ''}
